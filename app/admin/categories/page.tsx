...
export default function CategoriesPage() {
  ...
  const handleEdit = async (id: string, updatedCategory: Omit<Category, "id">) => {
    try {
      await new Promise((resolve) => setTimeout(resolve, 300))
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      const updatedProducts = products.filter((product) => product.categoryId !== id)
      ...
      const deletedProductsCount = products.filter((p) => p.categoryId === id).length
      if (deletedProductsCount > 0) {
        showMessage(`Category and ${deletedProductsCount} associated product(s) deleted successfully!`)
      } else {
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        <h3 className="text-xl font-semibold text-gray-900 mb-4">All Categories ({categories.length})</h3>
        <CategoryList
